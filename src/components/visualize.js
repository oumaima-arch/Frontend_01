...
function ThreeScene() {
  ...
  // const [mesh_1, setMesh] = useState(null);
  ...
  const handlePredictModel = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:8000/model/vtp-file', {
        responseType: 'blob',
      });
  ...
  return (
    <div>
      <form>
        ...
        <button type="button" onClick={handleColorizeModel} disabled={!model || labels.length === 0}>
          Colorize
        </button>
